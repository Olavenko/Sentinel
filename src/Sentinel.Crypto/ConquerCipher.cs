using System;

namespace Sentinel.Crypto;

/// <summary>
/// CO 7xxx gameplay cipher — a self-inverse, dual-table XOR + nibble-swap transform.
/// <para>
/// Each byte is XOR'd against two rolling lookup tables (TableA and TableB), then
/// nibble-swapped and XOR'd with 0xAB. Because the operation is its own inverse,
/// the same <see cref="Transform"/> call decrypts ciphertext and encrypts plaintext.
/// </para>
/// <para>
/// Two independent 8-bit counters (<c>counterA</c>, <c>counterB</c>) index into the
/// tables and advance after each byte. <c>counterB</c> increments only when
/// <c>counterA</c> wraps around 0xFF, giving a 65 536-byte key cycle.
/// </para>
/// </summary>
public static class ConquerCipher
{
    /// <summary>
    /// Primary lookup table (256 bytes), indexed by <c>counterA</c>.
    /// </summary>
    public static ReadOnlySpan<byte> TableA => new byte[]
    {
        ,
    };

    /// <summary>
    /// Secondary lookup table (256 bytes), indexed by <c>counterB</c>.
    /// </summary>
    public static ReadOnlySpan<byte> TableB => new byte[]
    {
        ,
    };

    /// <summary>
    /// Transforms (encrypts or decrypts) <paramref name="length"/> bytes of
    /// <paramref name="buffer"/> in-place starting at <paramref name="offset"/>.
    /// </summary>
    /// <remarks>
    /// The transform is self-inverse: applying it twice returns the original data.
    /// Per-byte steps:
    /// <list type="number">
    ///   <item><description><c>b ^= TableA[counterA]</c></description></item>
    ///   <item><description><c>b ^= TableB[counterB]</c></description></item>
    ///   <item><description>Advance counters: <c>counterA</c> wraps at 0xFF and triggers <c>counterB</c> increment.</description></item>
    ///   <item><description><c>b = ((b &lt;&lt; 4) | (b &gt;&gt; 4)) ^ 0xAB</c> (nibble swap + XOR)</description></item>
    /// </list>
    /// </remarks>
    /// <param name="buffer">Buffer to transform.</param>
    /// <param name="offset">Start offset within <paramref name="buffer"/>.</param>
    /// <param name="length">Number of bytes to process.</param>
    /// <param name="counterA">Rolling index into <see cref="TableA"/>; updated in place.</param>
    /// <param name="counterB">Rolling index into <see cref="TableB"/>; updated in place.</param>
    public static void Transform(byte[] buffer, int offset, int length, ref int counterA, ref int counterB)
    {
        ReadOnlySpan<byte> tableA = TableA;
        ReadOnlySpan<byte> tableB = TableB;

        int end = offset + length;
        for (int i = offset; i < end; i++)
        {
            byte b = buffer[i];
            b ^= tableA[counterA];
            b ^= tableB[counterB];

            if (++counterA > 0xFF)
            {
                counterA = 0;
                if (++counterB > 0xFF)
                    counterB = 0;
            }

            buffer[i] = (byte)(((b << 4) | (b >> 4)) ^ 0xAB);
        }
    }

    /// <summary>
    /// Decrypts a server-to-client packet using the current session counters.
    /// </summary>
    /// <remarks>
    /// Returns a new array; the original is not modified.
    /// Both counters advance by <paramref name="encrypted"/>.Length bytes.
    /// </remarks>
    /// <param name="encrypted">Raw ciphertext received from the server.</param>
    /// <param name="counterA">Current TableA counter for this session; updated in place.</param>
    /// <param name="counterB">Current TableB counter for this session; updated in place.</param>
    /// <returns>Decrypted plaintext as a new byte array.</returns>
    public static byte[] DecryptServerPacket(byte[] encrypted, ref int counterA, ref int counterB)
    {
        byte[] result = (byte[])encrypted.Clone();
        Transform(result, 0, result.Length, ref counterA, ref counterB);
        return result;
    }

    /// <summary>
    /// Decrypts a client-to-server packet using fresh counters starting at (0, 0).
    /// </summary>
    /// <remarks>
    /// Returns a new array; the original is not modified.
    /// Counters always start at zero for client packets (per protocol convention).
    /// </remarks>
    /// <param name="encrypted">Raw ciphertext captured from the client.</param>
    /// <returns>Decrypted plaintext as a new byte array.</returns>
    public static byte[] DecryptClientPacket(byte[] encrypted)
    {
        byte[] result = (byte[])encrypted.Clone();
        int a = 0, b = 0;
        Transform(result, 0, result.Length, ref a, ref b);
        return result;
    }
}
